  ...
  jquery(function() {
    ...
    Dragfn.prototype = {
      init: function(obj) {
        ...
        self.moving = false;
        self.moves = '';
        self.searchWidth = window.screen.availWidth;
        self.searchHeight = window.screen.availHeight;

        //父级
        // self.box =
        //   $.type(self.options.scope) === 'string' ? self.options.scope : null;
        self.box = window;
        //手柄
        ...
        //三个事件
        self.handle.on(downEvent, function(ev) {
          self.start(ev, obj);
          obj.setCapture && obj.setCapture();
          return false;
        });
        if (self.options.dragChange) {
          $(obj).on(moveEvent, function(ev) {
            self.move(ev, obj);
          });
          $(obj).on(upEvent, function(ev) {
            self.end(ev, obj);
          });
        } else {
          $(document).on(moveEvent, function(ev) {
            self.move(ev, obj);
          });
          $(document).on(upEvent, function(ev) {
            self.end(ev, obj);
          });
        }
      },
          ...
          sortBox: function(obj) {
            ...
            for (var i = 0; i < arr.length; i++) {
              for (var j = i + 1; j < arr.length; j++) {
                if (
                  Number(arr[i].attr('index')) > Number(arr[j].attr('index'))
                ) {
                  var temp = arr[i];
                  arr[i] = arr[j];
                  arr[j] = temp;
      ...
      start: function(ev, obj) {
        ...
        var oEvent = ev || event;
        self.disX =
          (oEvent.clientX || oEvent.originalEvent.changedTouches[0].clientX) -
          obj.offsetLeft;
        self.disY =
          (oEvent.clientY || oEvent.originalEvent.changedTouches[0].clientY) -
          obj.offsetTop;
      ...
      move: function(ev, obj) {
        ...
        if (self.box != null) {
          ...
          if (l + rect.width > rule.lmax) {
            l = rule.lmax - rect.width;
          } else if (l < rule.lmin) {
            l = rule.lmin;
          }
          if (t + rect.height > rule.tmax) {
            t = rule.tmax - rect.height;
          } else if (t < rule.tmin) {
            t = rule.tmin;
          }
        }
        if (self.options.axis == 'all') {
          const rect = self.grid(obj, l, t);
        ...
        if (self.options.changeMode == 'sort' && self.options.pos == true) {
          ...
          self.options.changeMode == 'point' &&
          self.options.pos == true
        ) {
          self.pointDrag(obj);
        }
        if (self.options.pos == true) {
      ...
      //算父级的宽高
      collTestBox: function(obj) {
        var self = this;
        var l1 = 0;
        var t1 = 0;
        var l2 = $(window).innerWidth() || $(window).outerWidth();
        var t2 = $(window).innerHeight() || $(window).outerHeight();
        return {
          lmin: l1, //取的l最小值
          tmin: t1, //取的t最小值
          lmax: l2, //取的l最大值
      ...
      sort: function() {
        ...
        for (var i = 0; i < arr_li.length; i++) {
          for (var j = i + 1; j < arr_li.length; j++) {
            if (
              Number($(arr_li[i]).attr('index')) >
              Number($(arr_li[j]).attr('index'))
            ) {
              var temp = arr_li[i];
              arr_li[i] = arr_li[j];
              arr_li[j] = temp;
        ...
        if (oNear) {
          if (Number($(oNear).attr('index')) > Number($(obj).attr('index'))) {
            ...
            for (var i = obj_tmp; i < Number($(oNear).attr('index')) + 1; i++) {
              ...
              $(arr_li[i]).attr(
                'index',
                Number($(arr_li[i]).attr('index')) - 1
              );
            }
          } else if (
            Number($(obj).attr('index')) > Number($(oNear).attr('index'))
          ) {
            //后换前
            var obj_tmp = Number($(obj).attr('index'));
            $(obj).attr('index', $(oNear).attr('index'));
